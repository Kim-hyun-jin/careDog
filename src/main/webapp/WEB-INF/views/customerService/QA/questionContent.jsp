		...
		<div class="row">
			...
			<div class="col-lg-10">
				...
				<div class="mb-3">
					...
					<input type="text" readonly="readonly" class="form-control" id="exampleFormControlInput2" value="${qadto.title }">
				</div>
				<div class="mb-3">
				...
				<a class="btn btn-warning" href="/customerService/QA/question?currentPage=${currentPage }">목록보기</a>
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-footer">
								...
								<a href="/customerService/QA/questionUpdateForm?currentPage=${currentPage }&chk=1&questionNo=${qadto.questionNo}" class="btn btn-primary">수정하기</a>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="modal fade" id="staticBackdrop2" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">닫기</button>
								<a href="/customerService/QA/questionDelete?currentPage=${currentPage }&chk=1&questionNo=${qadto.questionNo}" class="btn btn-primary">삭제</a>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="mb-3">
						...
						<c:choose>
							...
							<c:otherwise>
									...
									<textarea class="form-control" readonly="readonly" name="answerContent" id="exampleFormControlTextarea2" rows="5"></textarea>
							</c:otherwise>
						</c:choose>
					</div>
				</c:if>
				<c:choose>
					<c:when test="${roleAdmin && qadto.role ne 'ROLE_ADMIN' && qadto.status == '답변 미완료' }">
						<a href="/customerService/QA/questionAdminUpdateForm?currentPage=${currentPage }&chk=1&questionNo=${qadto.questionNo}" class="btn btn-warning">답변하기</a>
					</c:when>
					<c:when test="${roleAdmin && qadto.role ne 'ROLE_ADMIN'}">
						<a href="/customerService/QA/questionAdminUpdateForm?currentPage=${currentPage }&chk=1&questionNo=${qadto.questionNo}" class="btn btn-warning">수정하기</a>
					</c:when>
				</c:choose>
			</div>
		</div>
